...
class Discriminator_tiny(nn.Module):
    ...
    def __init__(self, nc=3, ngf=64, t_emb_dim=128, act=nn.LeakyReLU(0.2), patch_size=1, use_local_loss=False, num_layers=4):
        ...
        self.conv3 = DownConvBlock(ngf * 4, ngf * 4, t_emb_dim=t_emb_dim, downsample=True, act=act)

        self.conv4 = None
        if num_layers >= 4:
            self.conv4 = DownConvBlock(ngf * 8, ngf * 8, t_emb_dim=t_emb_dim, downsample=True, act=act)

        self.final_conv = conv2d(ngf * 4 + 1, ngf * 4, 3, padding=1, init_scale=0.)
        self.end_linear = dense(ngf * 4, 1)
        if use_local_loss:
            self.local_end_linear = dense(ngf * 4, 1)
...
class Discriminator_small_raD(nn.Module):
    """A time-dependent discriminator for small images (CIFAR10, StackMNIST)."""
